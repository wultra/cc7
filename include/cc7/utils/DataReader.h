...
public:
     ...
     */
    bool readData(ByteArray & out_data, size_t expected_size = 0);
     ...
     */
    bool readMemory(ByteArray & out_data, size_t size);
    
    /**
     Similar to readData(), but returns sub-range to internal data object.
     */
    bool readRange(ByteRange & out_range, size_t expected_size = 0);

    /**
     Similar to readMemory() method, but returns just a sub-range to
     internal data object;
     */
    bool readMemoryRange(ByteRange & out_range, size_t size);
