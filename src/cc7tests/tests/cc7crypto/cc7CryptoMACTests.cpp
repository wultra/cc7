...
public:
    ...
    void testMAC()
    {
        auto root = JSON_ParseFile(g_testFiles, "test-data/mac-test-data.json");
        ...
        for (const auto & item : data) {
            ...
            for (const auto & test_data : test_vectors) {
                ...
                auto mac = crypto::MAC::getInstance(algorithm);
                //
                ...
                if (expected_token != token) {
                    ...
                    ccstMessage("%s: exp  : %s", algorithm.c_str(), expected_token.hexString().c_str());
                }
            }
        }
    }
