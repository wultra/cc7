...
void DataReader::resetWithNewByteRange(const ByteRange & range)
{
    _offset = 0;
    _data.assign(range);
    _data_copy.clear();
...
void DataReader::resetWithNewByteArray(const ByteArray &data)
{
    _offset = 0;
    _data_copy.assign(data);
    _data.assign(_data_copy.byteRange());
...
bool DataReader::readMemory(ByteArray & out_data, size_t size)
{
    if (!canReadSize(size)) {
        return false;
    }
...
bool DataReader::readRange(ByteRange & out_range, size_t expected_size)
{
    size_t size;
    if (!readCount(size)) {
        return false;
...
bool DataReader::readMemoryRange(ByteRange & out_range, size_t size)
{
    if (!canReadSize(size)) {
        return false;
    }
